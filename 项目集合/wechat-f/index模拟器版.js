 ...
 * @Last Modified time: 2023-01-29 23:33:42
 */
//定义全局变量
let window;
let sub_log = true;
...
ui.layout(
    <vertical padding="15">
        ...
        <horizontal>
            ...
            <input id="dtime" inputType="number" gravity="center" w="80" text="1" />
...
function core() {
    ...
    StartApp()
    log('::点击发现')
    // fullId = com.tencent.mm:id/dtf
    // text('发现').findOnce().parent().click()
    FindClickUi('com.tencent.mm:id/dtf', true)
    sleep(random(2000, 3000))

    log('::点击放大镜')
    // FindClickUi('com.tencent.mm:id/gsl', true)
    FindClickUi('com.tencent.mm:id/dt5', true)
    ...
    log('::点击视频号')
    while (!className("android.view.View").text("视频号").exists()) {
        log("::等待视频号按钮加载");
        sleep(1000)
        if (exitFlag) {
            log('::主动退出脚本')
    ...
    let sphBut = className("android.view.View").text("视频号").findOnce()
    if (sphBut) {
    ...
    while (true) {
        ...
        log('::点击进入主页')
        let sNum = 0
        // className("android.view.View").text("牛了个牛美食甄选店").findOne().click()
        while (!className("android.view.View").text(numName).exists() && sNum < 8) {
            log('::暂未搜索到指定视频号主,继续等待搜索')
            sleep(random(1000, 2000))
            sNum++;
            continue;
        }
        if (!className("android.view.View").text(numName).exists() && sNum >= 8) {
            NetDelVu(numName)
            log('::8次都未搜到指定视频号主,上报移除:' + numName)
            continue;
        }
        while (className("android.view.View").text(numName).exists()) {
            let au = className("android.view.View").text(numName).findOnce()
            let tmp2 = au.bounds()
            click(tmp2.centerX(), tmp2.centerY())
            log('::没有进去主页再进入一次')
            sleep(random(1000, 2000))
...
function LoveClick(lx) {
    if (lx[0].parent() != null && lx[0].parent().parent() != null) {
        ...
        let timeText = lx[i].parent().parent().findOne(id('com.tencent.mm:id/bk7'))
        if (name == sName) {
            log('::作者评论不点赞')
        } else {
            if (time.includes('小时') || time.includes('分钟') || timeText.textStartsWith('1天前') || timeText.textStartsWith('2天前') || timeText.textStartsWith('3天前')) {
                //去重
