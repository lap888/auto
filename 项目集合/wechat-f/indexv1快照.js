 ...
 * @Last Modified time: 2023-01-09 15:05:42
 */
//定义全局变量
let sub_log = true;
let h = device.height;
...
ui.customize.click(function () {
    ...
    thread = threads.start(function () {
        ...
        log('操作间隔：' + dtime / 1000 + '秒' + sName + dzCountTotal + '次' + 'kk:' + kk)
        exitFlag = false;
        main();
    });
...
function LoveClick(lx) {
    if (lx[0].parent() != null && lx[0].parent().parent() != null) {
        ...
        if (name == sName) {
            ...
            if (time.includes('小时') || time.includes('分钟') || time.includes('天')) {
                //去重
                ...
                log('::评论超过一个月不点赞,' + name + '@' + time + '累计点赞次数' + dzCount)
            }
        }
    } else {
        log('父组件不在')
