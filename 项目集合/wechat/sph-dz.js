 ...
 * @Last Modified time: 2023-01-20 18:54:08
 */
...
ui.customize.click(function () {
    ...
    thread = threads.start(function () {
        ...
        toastLog('操作间隔：' + dtime / 1000 + '秒' + sName + dzCountTotal + '次')
        exitFlag = false;
        main();
    });
...
function dz() {
    while (true) {
        sleep(dtime)
        if (exitFlag) {
            toastLog('::主动退出脚本 点赞' + sName + '次数' + dzCount)
            break
        }
        ...
        if (sps.length > 1) {
            ...
            while (dzCount < dzCountTotal) {
                ...
                while (dzCount < dzCountTotal) {
                    ...
                    if (line.exists() || oldName == nowName) {
                        pls.forEach((v, i) => {
                            if (i > 1) {
