            ...
            while (true) {
                ...
                if (chk_userPage()) {
                    ...
                    if (chk_fansListPage()) {
                        ...
                        back()
                        sleep(dtime)
                        i_move(500, device.height - (device.width / 7))
                    }
                }
                if (chk_maxDown()) {
                    log('::到底跳出')
...
function fork_click() {
    ...
    if (find_click_ui('com.ss.android.ugc.aweme:id/user_avatar', true)) {
        ...
        if (find_click_ui('com.ss.android.ugc.aweme:id/u52', false)) {
            sleep(stime)
            log('找到了')
            return true;
        } else {
...
function foucs_click() {
    ...
    if (tmp_btn.exists()) {
        let btn_foucs = id("com.ss.android.ugc.aweme:id/" + idList[2]).visibleToUser().findOnce().bounds()
...
function fans_Listclick() {
    ...
    // fullId = android:id/text1
    // fullId = com.ss.android.ugc.aweme:id/grc
    let tmp_btn = id('com.ss.android.ugc.aweme:id/' + idList[8]).visibleToUser().findOnce()
    //let tmp_btn=id('com.ss.android.ugc.aweme:id/qb0').visibleToUser().findOnce()
    if (tmp_btn) {
        if (sub_log) { log('点击“粉丝作品列表item[0]”') }
        let tmp_pos = tmp_btn.bounds()
        click(tmp_pos.centerX(), tmp_pos.centerY())
